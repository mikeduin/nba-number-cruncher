exports.up = function(knex, Promise) {
  return knex.schema.alterTable('players_l10', (t) => {
    t.float('fgm_new'); // create new column of type float
    t.float('fga_new'); // create new column of type float
  })
  .then(() => {
    return knex.raw(`UPDATE players_l10 SET fgm_new = CAST(fgm AS FLOAT), fga_new = CAST(fga AS FLOAT);`); // copy data from old columns to new
  })
  .then(() => {
    return knex.schema.alterTable('players_l10', (t) => {
...
exports.down = function(knex, Promise) {
  ...
  .then(() => {
    return knex.raw(`UPDATE players_l10 SET fgm_new = CAST(fgm AS INTEGER), fga_new = CAST(fga AS INTEGER);`);
  })
  .then(() => {
    return knex.schema.alterTable('players_l10', (t) => {
      t.dropColumn('fgm');
