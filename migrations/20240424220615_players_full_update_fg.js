exports.up = function(knex, Promise) {
  ...
  .then(() => {
    return knex.raw(`UPDATE players_full SET fgm_new = CAST(fgm AS FLOAT), fga_new = CAST(fga AS FLOAT);`); // copy data from old columns to new
  })
  .then(() => {
    return knex.schema.alterTable('players_full', (t) => {
...
exports.down = function(knex, Promise) {
  ...
  .then(() => {
    return knex.raw(`UPDATE players_full SET fgm_new = CAST(fgm AS INTEGER), fga_new = CAST(fga AS INTEGER);`);
  })
  .then(() => {
    return knex.schema.alterTable('players_full', (t) => {
      t.dropColumn('fgm');
