...
const { formPlayerBaseStatsBuild } = require("../utils/nbaApi/formPlayerBaseStatsBuild");
const { formPlayerAdvancedStatsBuild } = require("../utils/nbaApi/formPlayerAdvancedStatsBuild");
...
module.exports = {
  ...
  updateTeamDbBaseStats: (db, arrayData, headers) => {
  ...
  updateTeamDbAdvancedStats: (db, arrayData, headers) => {
    arrayData.forEach(async team => {
      const checkExist = await knex(db).where({ team_id: team[headers.indexOf('TEAM_ID')] });
      if (!checkExist.length) {
        try {
