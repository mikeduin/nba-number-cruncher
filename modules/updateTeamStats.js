...
const updateFullTeamBuild = (games, db, period) => {
  axios.get(advancedTeamStats, {params: dbBuilders.fetchAdvancedTeamParams(games, period), headers: headers})
    .then(response => {
...
const updatePartialTeamBuild = (games, db, lineup) => {
  axios.get(advancedTeamStats, {params: dbBuilders.fetchLineupParams(games, lineup), headers: headers})
    .then(response => {
      let teamData = response.data.resultSets[0].rowSet;
      // Change this to .buildTeabDb for initial builds
      // dbBuilders.buildTeamDb(db, teamData);
