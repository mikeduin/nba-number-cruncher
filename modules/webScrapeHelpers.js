...
import { findByName } from "../modules/teamLookup.js";
...
export const sbLineParser = (line) => {
  let parsed = {};
  ...
  parsed.hAbb = nullChecker(findByName(parsed.hSplit[parsed.hSplit.length-1]).a);
  parsed.aSplit = nullChecker(line.awayTeam.split(' '));
  parsed.aAbb = nullChecker(findByName(parsed.aSplit[parsed.aSplit.length-1]).a);
  ...
  parsed.home_id = findByName(parsed.hSplit[parsed.hSplit.length-1]).id;
  parsed.away_id = findByName(parsed.aSplit[parsed.aSplit.length-1]).id;
  return parsed;
};
